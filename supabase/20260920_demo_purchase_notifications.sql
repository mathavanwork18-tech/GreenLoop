-- ==============================================================================
-- GREEN LOOP — FINAL BUY FLOW FIX & NOTIFICATIONS MIGRATION
-- ==============================================================================
-- File: supabase/20260920_demo_purchase_notifications.sql
--
-- Supports all 4 Role Combinations without restriction:
--   1. General -> General (Buyer: General, Seller: General)
--   2. General -> Local Shop (Buyer: General, Seller: Local Shop)
--   3. Local Shop -> General (Buyer: Local Shop, Seller: General)
--   4. Local Shop -> Local Shop (Buyer: Local Shop, Seller: Local Shop)
--
-- Rules:
--   - Only restriction: buyer_id != seller_id (User cannot buy own item)
--   - Legacy post_claims 'pending' status does NOT block the purchase
--   - Atomic row-locking (FOR UPDATE) guarantees single winner if simultaneous purchases occur
--   - Asking price validated strictly against database asking_price
--   - Post status transitioned to 'sold'
--   - Seller receives notification: "Your product was sold"
--   - Buyer receives notification: "Payment Successful"
--   - Buyer awarded +10 Green Coins, Seller awarded +20 Green Coins
-- ==============================================================================

-- 1. Ensure marketplace_transactions table exists with buyer_role and seller_role
...
-- Ensure marketplace_purchases table constraints accept DEMO_QR and SUCCESS (for legacy views)
DO $$
BEGIN
